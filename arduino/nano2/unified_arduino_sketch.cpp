/*
 * Arduino Nano 2 (GRÜN) - Tonhöhe und Programmauswahl
 * 
 * Hardware:
 *   - 11-Position Drehschalter an Pins 2-12
 *   - Potentiometer an A0 (für Tonhöhe)
 *   - Interrupt-Pin: Pin 15 (A1) -> Raspberry Pi GPIO 17
 *   - LED an Pin 13 (GRÜN)
 *   - I2C-Adresse: 0x09
 * 
 * Funktion:
 *   - Liest Schalterposition (1-11 = Hauptprogramme)
 *   - Liest Potentiometer für Tonhöhenregelung
...
// KONFIGURATION FÜR NANO 2 (GRÜN)
#define I2C_SLAVE_ADDRESS 0x09    // I2C-Adresse für Program-Arduino
#define INTERRUPT_PIN 15           // A1 -> Raspberry Pi GPIO 17
#define POTENTIOMETER_PIN A0       // Tonhöhe-Potentiometer
#define LED_PIN 13                 // GRÜNE LED
...
void setup() {
  ...
  Serial.begin(9600);
  Serial.println("=== Arduino Nano 2 (GRÜN) ===");
  Serial.println("Funktion: Tonhöhe und Programmauswahl");
  Serial.print("I2C-Adresse: 0x");
  Serial.println(I2C_SLAVE_ADDRESS, HEX);
  Serial.println("Interrupt-Pin: 15 (A1) -> GPIO 17");
  ...
  Serial.println("💿 Warte auf Programm-Änderungen...");
  Serial.println("🎵 Drehe Potentiometer für Tonhöhe");
}
...
void loop() {
  ...
  if (switchValue != lastSwitchValue) {
    ...
    // GRÜNE LED: an bei Position 6-11, aus bei 1-5
    if (switchValue >= 6 && switchValue <= 11) {
    ...
    // Debug-Ausgabe
    Serial.print("💿 Neue Programm-Position: ");
  ...
  if (millis() - lastPotCheck > 100) {
    ...
    if (abs(potValue - potentiometerValue) > 10) {
      ...
      sendInterrupt();  // Auch Tonhöheänderung melden
      
      // Debug-Ausgabe
      Serial.print("🎵 Tonhöhe-Potentiometer: ");
