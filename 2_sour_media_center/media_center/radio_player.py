...
class IntegratedRadioPlayer:
    ...
    def _init_vlc(self):
        ...
        # Karte 3 = HifiBerry AMP2
        audio_options = [
            ['--no-video', '--aout=alsa', '--alsa-audio-device=plughw:3,0'],
            ['--no-video', '--aout=alsa', '--alsa-audio-device=hw:3,0'],
            ['--no-video', '--aout=alsa'],
            ['--no-video'],
        ]
...
def main():
    ...
    if i2c_available:
        ...
        # Fallback: Erste Station starten falls noch keine gewählt
        if player.current_station is None and player.stations:
            first_key = sorted(player.stations.keys(), key=lambda x: int(x))[0]
            player.change_station(first_key)
